...
import { buildFrontierChartData, calculateFrontier, dateValue, formatDate, getTodayDateString, sortVendors, type Model, type VendorOrder } from './model-data'
import './App.css'
...
function App() {
  ...
  const timelineBounds = useMemo(() => {
    if (filteredModels.length === 0) return { start: undefined, end: undefined }
    ...
    const end = latest ? (today > latest ? today : latest) : undefined
    return { start, end }
  }, [filteredModels])
  const timelineStart = timelineBounds.start
  const chartEnd = timelineBounds.end
  const chartData = useMemo(() => buildFrontierChartData(frontier, chartEnd), [frontier, chartEnd])
  ...
  const timelineDomain: [number, number] | undefined = timelineStart && chartEnd
    ? [dateValue(timelineStart), dateValue(chartEnd)]
    : undefined
