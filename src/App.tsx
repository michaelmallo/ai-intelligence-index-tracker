...
function App() {
  ...
  const chartData = useMemo(() => frontier.map((point) => ({ dateValue: point.dateValue, frontier: point.score })), [frontier])
  const vendorPoints = useMemo(() => activeVendors.map((vendor) => ({ vendor, color: vendorColors[options.vendors.indexOf(vendor) % vendorColors.length], data: data.models.filter((model) => model.vendor === vendor).map((model) => ({ ...model, dateValue: dateValue(model.releaseDate) })) })), [activeVendors, data.models, options.vendors])
  ...
  const timelineDomain: [number, number] | undefined = timelineStart && timelineEnd
    ...
    : undefined
  const vendorLabel = allSelected ? 'All' : selectedVendors.length > 0 ? selectedVendors.join(', ') : 'None'
